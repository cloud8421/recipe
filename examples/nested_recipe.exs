defmodule NestedRecipe do
  ...
  defmodule Build do
    ...
    def clone_repo(state) do
      %{repo_url: repo_url, app_name: app_name, revision: revision} = state
    ...
    def create_new_revision(state) do
      %{app_name: app_name, artifact: artifact} = state
    ...
    def replace_revision(state) do
      %{app_name: app_name, revision_number: revision_number, slug_url: slug_url} = state
  ...
  defmodule Rollback do
    ...
    def steps, do: [:get_previous_revision, :replace_revision, :restart_service]

    def get_previous_revision(state) do
      %{app_name: app_name, revision_number: revision_number} = state
